...
float4	PS_Resize(VS_OUTPUT_POST IN, float4 v0 : SV_Position0,
	uniform Texture2D inputtex, uniform float srcsize, uniform float destsize, uniform bool first) : SV_Target
{
	float4	res;
...
float4	PS_BloomPostPass(VS_OUTPUT_POST IN, in float4 table1 : TEXCOORD1, in float4 table2 : TEXCOORD2, float4 vPos : SV_Position0) : SV_Target
{
	...
	float3 bloom0,bloom1,bloom2,bloom3,bloom4,bloom5; 

	bloom0 = RenderTarget512.Sample(Sampler1, IN.txcoord0.xy) * table1.x;
	bloom0*= Color0;

	bloom1= texBiquadratic(RenderTarget256, Sampler1, 256.0, 1.0/256.0, IN.txcoord0.xy) * table1.y;
	bloom1*= Color1;

	bloom2= texBiquadratic(RenderTarget128, Sampler1, 128.0, 1.0/128.0, IN.txcoord0.xy) * table1.z;
	bloom2*= Color2;

	bloom3= texBiquadratic(RenderTarget64, Sampler1, 64.0, 1.0/64.0, IN.txcoord0.xy) * table1.w;
	bloom3*= Color3;

	bloom4= texBiquadratic(RenderTarget32, Sampler1, 32.0, 1.0/32.0, IN.txcoord0.xy) * table2.x;
	bloom4*= Color4;

	bloom5= texBiquadratic(RenderTarget16, Sampler1, 16.0, 1.0/16.0, IN.txcoord0.xy) * table2.y;
	bloom5*= Color5;

	bloom.xyz = bloom0 + bloom1 + bloom2 + bloom3 + bloom4 + bloom5;
...
technique11 Draw <string UIName="Multipass bloom"; string RenderTarget="RenderTarget512";>
{
	pass p0
	{
		...
		SetPixelShader(CompileShader(ps_5_0, PS_Resize(TextureDownsampled, 1024.0, 512.0, true)));
	}
}
...
technique11 Draw1 <string RenderTarget="RenderTarget256";>
{
	...
	{
		SetVertexShader(CompileShader(vs_5_0, VS_Quad()));
		SetPixelShader(CompileShader(ps_5_0, PS_Resize(RenderTarget512, 512.0, 256.0, false)));
	}
}
...
technique11 Draw2 <string RenderTarget="RenderTarget128";>
{
	...
	{
		SetVertexShader(CompileShader(vs_5_0, VS_Quad()));
		SetPixelShader(CompileShader(ps_5_0, PS_Resize(RenderTarget256, 256.0, 128.0, false)));
	}
}
...
technique11 Draw3 <string RenderTarget="RenderTarget64";>
{
	...
	{
		SetVertexShader(CompileShader(vs_5_0, VS_Quad()));
		SetPixelShader(CompileShader(ps_5_0, PS_Resize(RenderTarget128, 128.0, 64.0, false)));
	}
}
...
technique11 Draw4 <string RenderTarget="RenderTarget32";>
{
	...
	{
		SetVertexShader(CompileShader(vs_5_0, VS_Quad()));
		SetPixelShader(CompileShader(ps_5_0, PS_Resize(RenderTarget64, 64.0, 32.0, false)));
	}
}
...
technique11 Draw5 <string RenderTarget="RenderTarget16";>
{
	...
	{
		SetVertexShader(CompileShader(vs_5_0, VS_Quad()));
		SetPixelShader(CompileShader(ps_5_0, PS_Resize(RenderTarget64, 32.0, 16.0, false)));
	}
}

...
<
    string UIName="--> Click here to enable Mouse Focus";
> = {false};

bool    text0            
<
    string UIName="-------------------------- DOF Near Field settings ---------------------------------------------------";
> = {false};
...
bool    DOFAnamorphic             
<
    string UIName="--> DOF : Anamorphic enable";
> = {false};

float   DOFAnamorphicStretch
<
    string UIName="--> DOF : Anamorphic stretch";
    string UIWidget="spinner";
    float UIMin=0.0;
    float UIMax=4.0;
> = {1.707};

float   DOFDistribution
<
    string UIName="--> DOF : Distribution";
    string UIWidget="spinner";
    float UIMin=1.0;
    float UIMax=4.0;
> = {1.0};
...
bool    textmouse            
<
    string UIName="-------------------------- Misc. settings ---------------------------------------------------";
> = {false};

float   LensDistortionAmount
<
    string UIName="--> Lens Distortion : Distortion Amount";
    string UIWidget="spinner";
    float UIMin=0.0;
    float UIMax=1.0;
> = {0.0};

bool   LensDistortionAlwaysZoomed
<
    string UIName="--> Lens Distortion : Always zoomed";
> = {true};

float  LensDistortionZoomAmount
<
    string UIName="--> Lens Distortion : Zoom Amount";
    string UIWidget="spinner";
    float UIMin=0.0;
    float UIMax=1.0;
> = {0.0};
...
float3 GetDOF(float2 theUV, float theDepth, float theFocus)
{
    ...
    if (DOFAnamorphic) pixelres.y *= DOFAnamorphicStretch;
    ...
    for (float d = 0.0f; radius < MaxSize; d += 2.3999f)
    {
        ...
        color  += lerp(color / weight, dofcolor, mask * DOFDistribution);
...
float4  PS_Dof2(VS_OUTPUT_POST IN, float4 v0 : SV_Position0) : SV_Target
{
    ...
    for(int i = 0; i < steps; i++)
    {
        ...
        dir.y *= ScreenSize.z;
        if (DOFAnamorphic) dir.y *= DOFAnamorphicStretch;
...
float4  PS_LensDistortion(VS_OUTPUT_POST IN, float4 v0 : SV_Position0) : SV_Target
{    
	float3 color = 0.0f; 
	float2 uv = 0.0f;

	float isDistorted = LensDistortionAlwaysZoomed ? 1.6f : LensDistortionZoomAmount;

    uv.xy  = (IN.txcoord0.xy - 0.5f) * 2.0f;
    uv.y  *= 0.75f;
    uv.xy /= 1.0f + dot(uv.xy, uv.xy) * -LensDistortionAmount + isDistorted * LensDistortionAmount;
    uv.y  *= 1.333f;
    uv.xy  = uv.xy * 0.5f + 0.5f;

    color = TextureColor.Sample(Sampler1, uv.xy).xyz;

	if (uv.x < 0 || uv.x > 1 || uv.y < 0 || uv.y > 1) color.xyz = 0.0f;
